...
@Component({
  ...
  styleUrls: ['./admin-pet-foods.component.css']
})
export class AdminPetFoodsComponent implements OnInit {
  ...
  updatePetFood() {
    if (this.editingPetFood) {
      this.petfoodService.updatePetFood(this.editingPetFood.foodId, this.editingPetFood).subscribe(
  ...
  searchById() {
    ...
    if (id > 0) {
      this.petfoodService.getPetFoodById(id).subscribe(
        (data: PetFood) => {
